...
int PVS(Board &b, int depth, int alpha, int beta, int threadID, bool isCutNode, SearchStackInfo *ssi, SearchPV *pvLine) {
    ...
    if (probeLimit
     ...
     && !b.getAnyCanCastle()) {
        ...
        if (tbProbeResult != 0) {
            ...
            HashData hashData(tbDepth, NULL_MOVE, adjustHashScore(tbScore, ssi->ply), PV_NODE, transpositionTable.getAge());
    ...
    for (Move m = moveSorter.nextMove(); m != NULL_MOVE;
        ...
        if (score >= beta) {
            ...
            HashData hashData(depth, m, adjustHashScore(score, ssi->ply), CUT_NODE, transpositionTable.getAge());
    ...
    if (prevAlpha < alpha && alpha < beta) {
        HashData hashData(depth, toHash, adjustHashScore(alpha, ssi->ply), PV_NODE, transpositionTable.getAge());
        transpositionTable.add(b, hashData, depth);
    ...
    // Record all-nodes
        ...
        if (!isPVNode && hashed != NULL_MOVE) {
            HashData hashData(depth, hashed, adjustHashScore(bestScore, ssi->ply), ALL_NODE, transpositionTable.getAge());
            transpositionTable.add(b, hashData, depth);
        }
        // Otherwise, just store no best move as expected
        else {
            HashData hashData(depth, NULL_MOVE, adjustHashScore(bestScore, ssi->ply), ALL_NODE, transpositionTable.getAge());
            transpositionTable.add(b, hashData, depth);
        }
    }
...
int quiescence(Board &b, int plies, int alpha, int beta, int threadID) {
    ...
    for (Move m = nextMove(legalMoves, scores, i); m != NULL_MOVE;
        ...
        if (score >= beta) {
            HashData hashData(-plies, m, adjustHashScore(score, searchParams->ply + plies), CUT_NODE, transpositionTable.getAge());
