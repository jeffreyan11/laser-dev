...
int PVS(Board &b, int depth, int alpha, int beta, int threadID, bool isCutNode, SearchStackInfo *ssi, SearchPV *pvLine) {
    ...
    for (Move m = moveSorter.nextMove(); m != NULL_MOVE;
        ...
        if (moveIsPrunable
         && pruneDepth <= 2
         && ((ssi->counterMoveHistory != nullptr) ? ssi->counterMoveHistory[pieceID][endSq] : 0) < -100 * pruneDepth
         && ((ssi->followupMoveHistory != nullptr) ? ssi->followupMoveHistory[pieceID][endSq] : 0) < -100 * pruneDepth)
            continue;
